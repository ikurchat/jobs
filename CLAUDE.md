...
Выведи краткий отчёт: ✅ ок / ⚠️ замечание / ❌ критично — для каждого скилла.

При `sync memory` — найди правила в MEMORY.md, которые относятся к конкретным скиллам, но не включены в их SKILL.md. Покажи что куда перенести. Если SKILL.md read-only — сообщи.

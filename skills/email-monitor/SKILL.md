...
description: >
  ...
  "что от Модестова", "письма от ...", "покажи письмо",
...
Мониторинг рабочей почты Gmail (`dedkubus@gmail.com`), анализ содержимого, классификация по приоритету, предложение задач, подготовка ответов. Обучается на решениях owner'а.
...
```
GMAIL_EMAIL=dedkubus@gmail.com
...
| "что от Модестова", "покажи важные" | FETCH → фильтр |
...
🔴 Критичные (X):
  1. [Модестов] Тема письма — 💡 Создать задачу
  2. ...
...
3. **Маппинг control_loop:**
   - Модестов / руководство → up (boss_control)
...
### Модестов
- **ВСЁ от Модестова = максимальный приоритет (10/10)**
...
- **Baserow**: email_inbox (848121), email_feedback (848122), sender_profiles (848123)

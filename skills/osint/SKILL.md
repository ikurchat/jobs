...
### 7.0 Интерфейс бота (карта кнопок)

**После /start бот отправляет 3 сообщения:**
1. Приветствие + keyboard-кнопки: `Показать меню`, `Выбрать пользователя`
2. Стикер (без текста)
3. **Главное меню** — примеры запросов + inline-кнопки:

| Кнопка | callback data | Назначение |
|---|---|---|
| 📄 Поиск по неполным данным | `constructor` | Открыть конструктор ФИО |
| 🕵️ Мой профиль | `MENU/PROFILE` | Баланс, ID, дата регистрации |
| 🤖 Мои боты | `BOTS/` | Управление API-ботами |
| 🤝 Партнерская программа | `MENU/PARTNERS` | Реферальная программа |

**Профиль (после клика MENU/PROFILE):**

| Кнопка | callback data | Назначение |
|---|---|---|
| 💵 Пополнить | `balance/topup` | Пополнение баланса |
| 🔎 Купить запросы | `pay/requests` | Покупка пакетов запросов |
| 🚫 Скрытие данных | `closing/` | Скрытие своих данных из поиска |
| 👁 Отслеживание | `monitoring/` | Мониторинг поисков по вашим данным |
| 🎩 Связаться | `MENU/CONTACTS` | Контакты поддержки |
| ⬅️ Назад | `MENU/` | Вернуться в меню |
| ⚙️ | `setting/` | Настройки |
| 🔄 | `MENU/PROFILE` | Обновить профиль |

**Конструктор (после клика `constructor`):**
Главное меню РЕДАКТИРУЕТСЯ и показывает форму с полями.

| Кнопка | callback data | Назначение |
|---|---|---|
| Фамилия (или ✔ ЗНАЧЕНИЕ) | `constructor/lastname` | Ввод фамилии |
| Имя (или ✔ ЗНАЧЕНИЕ) | `constructor/firstname` | Ввод имени |
| Отчество (или ✔ ЗНАЧЕНИЕ) | `constructor/middlename` | Ввод отчества |
| День | `constructor/day` | День рождения |
| Месяц | `constructor/month` | Месяц рождения |
| Год | `constructor/year` | Год рождения |
| Возраст от | `constructor/age_from` | Мин. возраст |
| Возраст | `constructor/age` | Точный возраст |
| Возраст до | `constructor/age_to` | Макс. возраст |
| Место рождения | `constructor/birth_place` | Место рождения |
| 🗑 Сбросить | `constructor/reset` | **ОБЯЗАТЕЛЬНО** перед новым поиском |
| 🇷🇺 | `constructor/country` | Смена страны поиска |
| 🔎 Искать | `constructor/search` | Запуск поиска |

**ВАЖНО:** Конструктор ПОМНИТ данные предыдущего поиска. Кнопки заполненных полей показывают `✔ ЗНАЧЕНИЕ` вместо метки. Скрипт автоматически делает reset перед заполнением.

**Заполнение поля:**
1. Клик по кнопке поля → сообщение меняется на "Введите {поле}:" + кнопка "Отменить" (data=`constructor`)
2. Отправка значения текстом → сообщение возвращается к форме с `✔ ЗНАЧЕНИЕ`

**Результаты конструктора (пагинация):**
Одна запись на страницу: `` `ФИО ДД.ММ.ГГГГ` `` + возраст + место рождения

| Кнопка | callback data | Назначение |
|---|---|---|
| ⬅ | `constructor/view/{id}/{prev}` | Предыдущая страница |
| N / M | `noop` | Номер текущей / всего |
| ⮕ | `constructor/view/{id}/{next}` | Следующая страница |
| Начать новый поиск | `constructor` | Вернуться в конструктор |

**Выбор страны (после отправки ФИО+дата):**
Бот парсит ФИО и показывает меню стран:

| Кнопка | callback data | Назначение |
|---|---|---|
| Россия | `PERSON/RU/{id}` | Поиск по РФ |
| Казахстан | `PERSON/KZ/{id}` | Поиск по КЗ |
| Беларусь | `PERSON/BY/{id}` | Поиск по РБ |
| Украина | `PERSON/UA/{id}` | Поиск по УА |
| Узбекистан | `PERSON/UZ/{id}` | Поиск по УЗ |
| Туркменистан | `PERSON/TM/{id}` | Поиск по ТМ |

Скрипт автоматически выбирает "Россия". Fallback: первая кнопка с `PERSON/` в data.
...
### Шаг 7.2a — ФИО без даты рождения

**МАРШРУТ:** ФИО без ДР → конструктор (`fio_constructor`) → выбор из найденных → полный запрос по выбранному ФИО+дата.

Конструктор **НЕ тратит кредит** на поиск совпадений. Кредит списывается только при отправке полного ФИО+дата для получения досье.

**Каскад сбора данных:**

1. **Спросить пользователя:**
   ```
   tg_send_message: "Для точного пробива по ФИО нужна дата рождения. Есть ДР, телефон или город?"
   ```
   Если пользователь предоставил ДР — добавить к ФИО, отправить как обычный `fio` запрос.
   Если дал телефон — сначала пробить телефон, извлечь ДР, затем ФИО+ДР.

2. **Конструктор (если ДР нет):**
   ```bash
   cd /workspace/.claude/skills/osint && python3 osint_sherlock.py query "fio_constructor" "Фамилия Имя Отчество"
   ```
   Вернёт JSON с `entries_detail` — список совпадений с возрастом и местом рождения.
   Показать создателю для выбора → затем полный запрос.
...
### Шаг 7.4 — Пробив по ФИО (конструктор)

Вызов через `query_type = "fio_constructor"`:

```bash
cd /workspace/.claude/skills/osint && python3 osint_sherlock.py query "fio_constructor" "Панков Иван Юрьевич"
```

Скрипт автоматически:
1. Находит кнопку `constructor` (по callback data) или /start для меню
2. **Сбрасывает** предыдущие данные (клик `constructor/reset`)
3. Заполняет поля по callback data: `constructor/lastname`, `constructor/firstname`, и т.д.
4. Нажимает `constructor/search`
5. **Пролистывает все страницы** результатов (пагинация по `constructor/view/{id}/{page}`)
...
```json
{
  ...
  "total": 8,
  "entries": ["Панков Иван Юрьевич 04.05.1969", ...],
  "entries_detail": [
    {"fio_date": "...", "age": "56 лет", "birthplace": "г. Москва"},
    ...
  ]
}
```

**Режим A — ФИО + дата рождения (полные данные):**
Если есть дата рождения (`Иванов Иван Иванович 01.01.1990`), используй обычный `fio` тип — скрипт отправляет текст напрямую боту. Бот показывает выбор страны — скрипт автоматически выбирает "Россия" (PERSON/RU/).

**Обработка результатов конструктора:**

| Результат | Действие |
|---|---|
| 1 совпадение | Автоматически отправляет полное ФИО+дату как обычный запрос |
| Несколько совпадений | Показать создателю список с возрастом и местом рождения, ждать выбор |
| 0 совпадений | Уведомить создателя |
| Ошибка | Показать сырой текст |

**При нескольких совпадениях — показать создателю:**
```
tg_send_message: "Найдено {N} совпадений:\n1. Панков Иван Юрьевич 04.05.1969 (56 лет, г. Москва)\n2. Панков Иван Юрьевич 15.12.1999 (26 лет, Россия, гор. Курск)\n...\n\nУкажи номер."
```

После выбора:
```bash
cd /workspace/.claude/skills/osint && python3 osint_sherlock.py query "fio" "Панков Иван Юрьевич 15.12.1999"
```
...
| `"error": "constructor_button_not_found"` | Бот обновился. Попробовать /start → найти кнопку |
| `"phase": "constructor_results"` + entries | Показать создателю список, ждать выбор |
...
Полный отчёт Sherlock: {report_url}
```
...
**Ссылка на полный отчёт:** результат `osint_sherlock.py query` содержит поле `report_url` (извлекается из inline-кнопки "Отчёт"/"Report"). Если `report_url` присутствует — ОБЯЗАТЕЛЬНО добавить в конец отчёта:
```
Полный отчёт Sherlock: {report_url}
```

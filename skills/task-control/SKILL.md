...
description: >
  ...
  generate plans/reports, track regulatory deadlines, or get a briefing.
  ...
  Triggers (status update): "Кулиш сдал", "выполнено", "готово", "задача закрыта",
  ...
  Triggers (plan/report): "план на неделю", "еженедельный план", "отчёт за неделю",
  "сформируй план", "сформируй отчёт", "план на месяц", "ежемесячный план",
  "weekly plan", "weekly report", "monthly plan".
...
```
mode: PARSE_TASKS | STATUS_UPDATE | BRIEFING | SHIFT_QUERY | PLAN_GEN | REPORT_GEN | ANALYTICS | REGULATORY | BOSS_CONTROL | SCHEDULE_UPLOAD | SKILL_UPDATE
...
| Обновление статуса задачи ("Кулиш сдал", "выполнено", "перенести срок") | **STATUS_UPDATE** |
| Запрос сводки ("что у меня", "брифинг", "сводка") | **BRIEFING** |
| Вопрос о сменах ("кто на смене", "кто свободен", "нагрузка") | **SHIFT_QUERY** |
| Генерация плана ("план на неделю", "ежемесячный план") | **PLAN_GEN** |
| Генерация отчёта ("отчёт за неделю") | **REPORT_GEN** |
...
## 6. PLAN_GEN — Генерация плана

**6.1.** Определи период (неделя / месяц).

**6.2.** Загрузи из Baserow:
- Незакрытые задачи текущего периода (переносятся)
- Пункты вышестоящего плана (месячного для еженедельного)
- Регуляторные треки с дедлайнами в периоде
- Задачи от руководства (boss_control)
- Бэклог (одобренный owner'ом)
- Регулярные мероприятия (мониторинг, SOC, дежурства)

**6.3.** Сформируй черновик плана (JSON) и сгенерируй .docx:
```bash
cd /workspace/.claude/skills/task-control && PYTHONPATH=. python3 -m services.reporter weekly_plan --input /dev/shm/task-control/plan_data.json --output /dev/shm/task-control/plan.docx
```

**6.4.** Покажи owner'у текстовую сводку + отправь .docx через `mcp__jobs__tg_send_media`.

**6.5.** При подтверждении — сохрани пункты плана в Baserow (plan_items).

---

## 7. REPORT_GEN — Генерация отчёта

**7.1.** Загрузи план текущего периода (plan_items) и все задачи.

**7.2.** Для каждого пункта плана найди связанные задачи (linked_tasks или по корреляции).

**7.3.** Заполни колонку «Отметка о выполнении» из task.result и task.status.

**7.4.** Раздели на два раздела:
- **Плановые мероприятия** — пункты из plan_items
- **Дополнительные мероприятия** — задачи с is_unplanned=TRUE

**7.5.** Сгенерируй .docx:
```bash
cd /workspace/.claude/skills/task-control && PYTHONPATH=. python3 -m services.reporter weekly_report_with_unplanned --plan /dev/shm/task-control/plan.json --unplanned /dev/shm/task-control/unplanned.json --output /dev/shm/task-control/report.docx
```

**7.6.** Покажи текстовую сводку + отправь .docx.

### Правила недельного отчёта
- Включать только внутренние мероприятия: справки, поручения, совещания, взаимодействие с сотрудниками/руководством
- НЕ включать: доработки кода бота, настройки скиллов, техническую работу над ботом

---

## 8. ANALYTICS — Аналитика и аномалии
...
## 9. REGULATORY — Регуляторные треки
...
## 10. BOSS_CONTROL — Задачи от руководства
...
## 11. SCHEDULE_UPLOAD — Загрузка графика смен
...
## 12. SKILL_UPDATE — Правки скиллов
...
## 15. Action Items — главный принцип
